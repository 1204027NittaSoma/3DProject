...
public:
	...
	bool CollisionRay(KdCollider::RayInfo _rayInfo,
		std::list<std::shared_ptr<KdGameObject>> _objList,
		Math::Vector3& _pos);

	bool CollisionSphere(const KdCollider::SphereInfo& _sphereInfo,
		std::list<std::shared_ptr<KdGameObject>> _objList,
		float& _maxOverLap, Math::Vector3& _dir, Math::Vector3& _pos);

	bool CollisionSphere(const KdCollider::SphereInfo& _sphereInfo,
		std::list<std::shared_ptr<KdGameObject>> _objList,
		float& _maxOverLap, Math::Vector3& _dir, Math::Vector3& _pos, std::weak_ptr<KdGameObject> _hitObj);

	void SetRetHitObj(const KdCollider::CollisionResult& _ret)
	{
		m_ret = _ret;
	}

	const KdCollider::CollisionResult GetRetHitObj()const
	{
		return m_ret;
	}
...
protected:
	...
	KdCollider::CollisionResult m_ret;
